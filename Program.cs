...
namespace Assignment3RoughDraft
{
    class Program
    {
        ...
        static void TheForce(ref int credits)
        {
           ...
           while(guesscount < 10 && KeepGoing(incorrect))
           {
                if(CheckForceGuess(randomValues, guesscount, forceMove))
                {
                    Console.WriteLine($"You answered correctly! The next card was {randomCards[guesscount + 1]}");
                    correct++;
                    guesscount++;
                }
                else if(!CheckForceGuess(randomValues, guesscount, forceMove))
                {
                    Console.WriteLine($"You answered incorrectly. The next card was {randomCards[guesscount + 1]}");
                    incorrect++;
                    guesscount++;
                }
                if(KeepGoing(incorrect))
                {
                    Console.WriteLine($"Card on table: {randomCards[guesscount]} \nIs the next card higher or lower?  {correct}/10 guesses to win\n1) Higher\n2) Lower");
        ...
        static bool KeepGoing(int incorrect)
        {
            if(incorrect == 0)
            {
                return true;
        ...
        static bool CheckForceGuess(int[] randomValues, int guesscount, string forceMove)
        {
            if(forceMove == "1")
            {
                if(randomValues[guesscount] < randomValues[guesscount + 1]) return true;
                else return false;
            }
            else if(forceMove == "2")
            {
